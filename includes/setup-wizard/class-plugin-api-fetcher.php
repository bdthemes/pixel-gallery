...
class Plugin_Api_Fetcher {
     ...
     *
     * @param string $plugin_slug Plugin slug (e.g., 'pixel-gallery' or 'plugin-name/plugin-name.php')
    ...
    private static function fetch_from_api($plugin_slug) {
        ...
        $response = wp_remote_get($api_url, [
            'timeout' => 30,
            'user-agent' => 'Pixel Gallery Setup Wizard'
        ]);
    ...
    private static function get_cached_data($plugin_slug) {
        $cache_key = 'pg_plugin_data_' . md5($plugin_slug);
    ...
    public static function clear_all_cache() {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
                '_transient_timeout_pg_plugin_data_%'
            )
        );
    }
     ...
     *
     * @param string $plugin_file Plugin file path (e.g., 'pixel-gallery/pixel-gallery.php')
