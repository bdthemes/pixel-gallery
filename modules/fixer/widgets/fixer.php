...
class Fixer extends Module_Base {
	...
	var alwaysLb = false;
	var gridClass = 'pg-fixer-grid pg-grid';
	...
	#>
		...
		<# _.each( items, function( item, index ) {
			var itemClass = 'pg-fixer-item pg-item elementor-repeater-item-' + item._id;
		#>
			<div class="{{ itemClass }}">
				<# if ( item.item_hidden !== 'yes' ) { #>
										<div class="pg-fixer-image-wrap bdt-pg-img-mask">
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-fixer-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-fixer-img">
						<# } #>
					...
					<div class="pg-fixer-content-wrap">
						...
						<# if ( settings.show_title === 'yes' && item.title ) { #>
							...
							<{{{ ttag }}} class="pg-fixer-title">{{{ elementor.helpers.sanitize( item.title ) }}}</{{{ ttag }}}>
						<# } #>
						<# if ( settings.show_meta === 'yes' && item.meta ) { #>
							<div class="pg-fixer-meta">{{{ elementor.helpers.sanitize( item.meta ) }}}</div>
						<# } #>
						</div>
					</div>
					<# } #>
