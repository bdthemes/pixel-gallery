...
<?php
	...
	var gridClass = 'pg-mosaic-grid pg-grid';
	...
	#>
		...
		<# _.each( items, function( item, index ) {
			var itemClass = 'pg-mosaic-item pg-item elementor-repeater-item-' + item._id;
		#>
			<div class="{{ itemClass }}">
				<# if ( item.item_hidden !== 'yes' ) { #>
										<div class="pg-mosaic-image-wrap bdt-pg-img-mask">
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-mosaic-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-mosaic-img">
						<# } #>
					...
					<div class="pg-mosaic-content">
						<# if ( settings.show_date === 'yes' && item.date ) { #>
							<div class="pg-mosaic-date"><span>{{ item.date }}</span></div>
						<# } #>
						<# if ( settings.show_title === 'yes' && item.title ) { #>
							<# var ttag = elementor.helpers.validateHTMLTag( settings.title_tag || 'h3' ); #>
							<{{{ ttag }}} class="pg-mosaic-title">{{{ elementor.helpers.sanitize( item.title ) }}}</{{{ ttag }}}>
						<# } #>
						<# if ( settings.link_to !== 'none' && ( settings.link_target || 'whole_item' ) === 'only_button' && item.readmore_text ) { #>
							<div class="pg-mosaic-readmore">
<?php $this->print_content_template_item_link_prepare( 'mosaic' ); ?>
...
<?php $this->print_content_template_lightbox_overlay( 'mosaic' ); ?>
					<# } #>
				<# } #>
			</div>
		<# } ); #>
