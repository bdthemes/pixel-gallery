				...
				<# if ( item.item_hidden !== 'yes' ) { #>
						...
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-fabric-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-fabric-img">
						<# } #>
...
<?php $this->print_content_template_lightbox_overlay( 'fabric' ); ?>
					<# } #>
				<# } #>
			</div>
		<# } ); #>
