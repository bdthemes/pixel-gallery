...
<?php
	...
	var gridClass = 'pg-plex-grid pg-grid';
	...
	#>
		...
		<# _.each( items, function( item, index ) {
			var itemClass = 'pg-plex-item pg-item elementor-repeater-item-' + item._id;
		#>
			<div class="{{ itemClass }}">
				<# if ( item.item_hidden !== 'yes' ) { #>
										<div class="pg-plex-image-wrap bdt-pg-img-mask">
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-plex-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-plex-img">
						<# } #>
					...
					<div class="pg-plex-meta-wrap">
						<# if ( settings.show_meta === 'yes' && item.meta ) { #>
							<div class="pg-plex-meta">{{{ elementor.helpers.sanitize( item.meta ) }}}</div>
						<# } #>
					</div>
					<div class="pg-plex-content">
						<# if ( settings.show_title === 'yes' && item.title ) { #>
							<# var ttag = elementor.helpers.validateHTMLTag( settings.title_tag || 'h3' ); #>
							<{{{ ttag }}} class="pg-plex-title">{{{ elementor.helpers.sanitize( item.title ) }}}</{{{ ttag }}}>
						<# } #>
						<# if ( settings.link_to !== 'none' && ( settings.link_target || 'whole_item' ) === 'only_button' ) { #>
							<div class="pg-plex-readmore">
<?php $this->print_content_template_item_link_prepare( 'plex' ); ?>
...
<?php $this->print_content_template_lightbox_overlay( 'plex' ); ?>
					<# } #>
				<# } #>
			</div>
		<# } ); #>
