...
<?php
	...
	var gridClass = 'pg-plumb-grid pg-grid';
	...
	#>
		...
		<# _.each( items, function( item, index ) {
			var itemClass = 'pg-plumb-item pg-item elementor-repeater-item-' + item._id;
		#>
			<div class="{{ itemClass }}">
					...
					<div class="pg-plumb-content-wrap">
						<div class="pg-plumb-content">
							<# if ( settings.show_title === 'yes' && item.title ) { #>
								...
								<{{{ ttag }}} class="pg-plumb-title">{{{ elementor.helpers.sanitize( item.title ) }}}</{{{ ttag }}}>
							<# } #>
							<# if ( settings.show_text === 'yes' && item.text ) { #>
								<div class="pg-plumb-text">{{{ elementor.helpers.sanitize( item.text ) }}}</div>
							<# } #>
							<# if ( settings.link_to !== 'none' && ( settings.link_target || 'whole_item' ) === 'only_button' && item.readmore_text ) { #>
								<div class="pg-plumb-readmore">
<?php $this->print_content_template_item_link_prepare( 'plumb' ); ?>
...
<?php $this->print_content_template_item_link_wrap_close(); ?>
							...
							<# } #>
										...
										<div class="pg-plumb-image-wrap bdt-pg-img-mask">
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-plumb-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-plumb-img">
						<# } #>
...
<?php $this->print_content_template_lightbox_overlay( 'plumb' ); ?>
					<# } #>
				<# } #>
			</div>
		<# } ); #>
