...
<?php
	...
	var gridClass = 'pg-verse-grid pg-grid';
	...
	#>
		...
		<# _.each( items, function( item, index ) {
			var eff = ( settings.verse_layout_style && 'left' !== settings.verse_layout_style ) ? ' pg-verse-effect-' + settings.verse_layout_style : '';
			var itemClass = 'pg-verse-item pg-item elementor-repeater-item-' + item._id + eff;
		#>
			<div class="{{ itemClass }}">
				<# if ( item.item_hidden !== 'yes' ) { #>
										<div class="pg-verse-image-wrap bdt-pg-img-mask">
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-verse-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-verse-img">
						<# } #>
					...
					<div class="pg-verse-content">
						<# if ( settings.show_title === 'yes' && item.title ) { #>
							...
							<{{{ ttag }}} class="pg-verse-title">{{{ elementor.helpers.sanitize( item.title ) }}}</{{{ ttag }}}>
						<# } #>
						<# if ( settings.show_text === 'yes' && item.text ) { #>
							<div class="pg-verse-text">{{{ elementor.helpers.sanitize( item.text ) }}}</div>
						<# } #>
					</div>
					<# if ( settings.link_to !== 'none' && ( settings.link_target || 'whole_item' ) === 'only_button' && item.readmore_text ) { #>
						<div class="pg-verse-readmore">
<?php $this->print_content_template_item_link_prepare( 'verse' ); ?>
...
<?php $this->print_content_template_lightbox_overlay( 'verse' ); ?>
					<# } #>
				<# } #>
			</div>
		<# } ); #>
