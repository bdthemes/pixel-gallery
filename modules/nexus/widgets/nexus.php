...
<?php
	...
	var gridClass = 'pg-nexus-grid pg-grid';
	if ( settings.nexus_select_design && settings.nexus_select_design !== 'default' ) {
		gridClass += ' pg-nexus-design-' + settings.nexus_select_design;
	}
	...
	#>
		...
		<# _.each( items, function( item, index ) {
			var itemClass = 'pg-nexus-item pg-item elementor-repeater-item-' + item._id;
		#>
			<div class="{{ itemClass }}">
				<# if ( item.item_hidden !== 'yes' ) { #>
										<div class="pg-nexus-image-wrap bdt-pg-img-mask">
						<# if ( item.media_type === 'video' ) { #>
							<# if ( item.poster && item.poster.url ) { #>
								<img src="{{ item.poster.url }}" alt="{{ item.title }}" class="pg-nexus-img">
							<# } #>
						<# } else if ( item.image && item.image.url ) { #>
							<img src="{{ item.image.url }}" alt="{{ item.title }}" class="pg-nexus-img">
						<# } #>
					...
					<# if ( settings.show_title === 'yes' && item.title ) { #>
						...
						<{{{ ttag }}} class="pg-nexus-title">{{{ elementor.helpers.sanitize( item.title ) }}}</{{{ ttag }}}>
					<# } #>
					<# if ( settings.link_to !== 'none' && ( settings.link_target || 'whole_item' ) === 'only_button' && item.readmore_text ) { #>
						<div class="pg-nexus-readmore">
<?php $this->print_content_template_item_link_prepare( 'nexus' ); ?>
...
<?php $this->print_content_template_lightbox_overlay( 'nexus' ); ?>
					<# } #>
				<# } #>
			</div>
		<# } ); #>
