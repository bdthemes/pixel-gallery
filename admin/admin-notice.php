...
class Notices {
	...
	private function get_api_notices_data() {
		...
		if( isset($notices->api) && isset($notices->api->{'pixel-gallery'}) ) {
			return $notices->api->{'pixel-gallery'};
		}
	...
	private function is_notice_dismissed($notice_id) {
		$dismissed_notices = get_user_meta(get_current_user_id(), 'pg_dismissed_notices', true);
		...
		if (isset($dismissed_notices[$notice_id])) {
			...
			if (is_array($dismissal_data) && isset($dismissal_data['end_date'])) {
				...
				if ($current_date > $end_date) {
					...
					update_user_meta(get_current_user_id(), 'pg_dismissed_notices', $dismissed_notices);
					return false;
				}
	...
	private function is_notice_compatible_with_plugin($notice) {
		...
		$is_lite_active = $current_plugin_slug === 'pixel-gallery';
	...
	private function get_plugin_priority() {
		...
		if ($current_plugin_slug === 'pixel-gallery') {
			return 1;
		}
		
		// Pro version has lower priority
		if ($current_plugin_slug === 'pixel-gallery-pro') {
			return 2;
		}
	...
	private function are_both_plugins_installed() {
		...
		$lite_active = is_plugin_active('pixel-gallery/pixel-gallery.php');
		
		// Check if pro plugin is active
		$pro_active = is_plugin_active('pixel-gallery-pro/pixel-gallery-pro.php');
	...
	private function get_current_plugin_slug() {
		...
		$plugin_file = plugin_basename(BDTPG__FILE__);
		...
		<div class="<?php echo esc_attr($wrapper_classes); ?>" <?php echo $background_style ? 'style="' . $background_style . '"' : ''; ?>>
			...
			<div class="bdt-api-notice-content">
				<div class="bdt-plugin-logo-wrapper">
					<img height="auto" width="40" src="<?php echo esc_url(BDTPG_ASSETS_URL); ?>images/logo.svg" alt="Ultimate Post Kit Logo">
				</div>
	...
	private function store_notice_data($notice_id, $notice) {
		$stored_notices = get_user_meta(get_current_user_id(), 'pg_stored_notices', true);
		...
		update_user_meta(get_current_user_id(), 'pg_stored_notices', $stored_notices);
	}
	...
	?>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function () {
				setTimeout(function () {
					document.querySelectorAll('.notice.pixel-gallery-notice').forEach(function (notice) {
