jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.pixel-gallery-notice.is-dismissible .notice-dismiss', function () {
        $this = $(this).parents('.pixel-gallery-notice');
        var $id = $this.attr('id') || '';
        var $time = $this.attr('dismissible-time') || '';
        var $meta = $this.attr('dismissible-meta') || '';
        $.ajax({
            url: (window.PixelGalleryNoticeConfig && PixelGalleryNoticeConfig.ajaxurl) ? PixelGalleryNoticeConfig.ajaxurl : (typeof ajaxurl !== 'undefined' ? ajaxurl : ''),
            type: 'POST',
            data: {
                ...
                meta: $meta,
                time: $time,
                _wpnonce: PixelGalleryNoticeConfig.nonce
            }
        });
    });
